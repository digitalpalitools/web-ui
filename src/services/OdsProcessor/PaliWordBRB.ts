import * as PW from './PaliWord'

class PaliWord implements PW.PaliWordBase {
  ...
  tocId = () => `${this.pali1.replace(/\s/g, '')}`

  includeInDictionary = () => !!this.inEnglish

  includeInRootCsv = () => !!this.paliRoot

  sortKey = () => PW.padTrailingNumbers(this.pali1)
}

export const createBRBPaliWord: PW.PaliWordFactory = (x) => new PaliWord(x)
